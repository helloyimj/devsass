...
(function($) {
	...
	$(function() {
			...
			$lnbDepthItem.on('mouseover.menu focusin.menu', function(event) {
					...
					if (mode === 'mobile') {
							...
							if ($depthText.find(eventTarget).length || $depthText[0] === eventTarget) {
									...
									/*if($this.children('.depth').children().children('.depth_item').length) {
											$this.toggleClass('active').siblings('.depth_item').removeClass('active');
											event.preventDefault();
									}*/
					...
					if ($element.children('.depth').children().children('.depth_item').length) {
							$element.addClass('has');
					}
			});

			$lnbMenu.find('.depth1_item:last-child .depth2 .depth2_list .depth2_item:last-child .depth2_text').on('focusout.menu', function(event) {
					if (mode === 'pc') {
							$lnbMenu.height('');
							$html.removeClass('lnb_open');
							$lnbDepthItem.removeClass('active');
			...
			if (window.location.href.indexOf("index.do") > -1) {
					$lnbDepth1FirstChild.addClass('active');
			}
			...
			var $container = $('#container')
				...
				, $sideDepth1Item = $side.find('.depth1_item')
			...
			if ($sideDepth1Item.length === 0) {
					$('.side_menu').addClass('empty');
			}
	});
}
)(window.jQuery);
